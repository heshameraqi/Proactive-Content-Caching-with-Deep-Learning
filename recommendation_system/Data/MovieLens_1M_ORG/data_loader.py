...
class MovieLens1MDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, sep='::', engine='python', header=None):
        # Read the ratings--> [UserID::MovieID::Rating::Timestamp]
        ratings = pd.read_csv("ratings.dat", sep=sep, engine=engine, header=header).to_numpy()
        ratings = ratings[:, :3]  # discard timestamp info

        # Retrieve the items and ratings data
        self.items = ratings[:, :2].astype(np.int) - 1  # -1 because ID begins from 1
        self.targets = self.__preprocess_target(ratings[:, 2]).astype(np.float32) # ratings
