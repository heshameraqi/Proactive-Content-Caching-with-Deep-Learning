...
class Transformer(nn.Module):
    ...
    def forward(self, src):
        ...
        # Sinusoidal Positional Encoding
        patch_embed = torch.ones((bs, l), dtype=torch.bool, device=src.device)
        patch_embed = patch_embed.cumsum(1, dtype=torch.float32)
        # Normalize
        eps = 1e-6
        patch_embed = patch_embed / (patch_embed[:, -1:] + eps) * 2 * math.pi

        dim_t = torch.arange(self.num_pos_feats, dtype=torch.float32, device=src.device)
        ...
        pos_embed = pos_patch.permute(1, 0, 2)  # [L, N, C]
        pos_embed = None
