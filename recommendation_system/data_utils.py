...
def load_all(test_num=100):
	...
	# To create the implicit feedback --> matrix (n,m) where 1 is set to indicate if the user has ratted this item or not.
	train_mat = sp.dok_matrix((user_num, item_num), dtype=np.float32)
	for x in train_data:
		train_mat[x[0], x[1]] = 1.0

	test_data = []  # first item in the test_data is user-item which is rated
